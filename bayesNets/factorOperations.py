...
def joinFactors(factors):
    ...
    for i in unconditioned:
        if i in conditioned:
            conditioned.remove(i)
    ...
    for assignment in joined.getAllPossibleAssignmentDicts():
        joined.setProbability(assignment, 1.0)
        for factor in factors:
            joined.setProbability(assignment, joined.getProbability(assignment)*factor.getProbability(assignment))
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        eliminated = Factor(unconditioned, conditioned, factor.variableDomainsDict())

        for assignment in factor.getAllPossibleAssignmentDicts():
            p = factor.getProbability(assignment)
            assignment.pop(eliminationVariable)
            eliminated.setProbability(assignment, p+eliminated.getProbability(assignment))
        return eliminated
