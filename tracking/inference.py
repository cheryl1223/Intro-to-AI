...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        ghostPosition = self.allPositions
        for ghost in ghostPosition:
            update = {ghost:self.getObservationProb(noisyDistance, pacmanPosition, ghost, jailPosition)}
            emissionModel.update(update)

        for location in self.allPositions:
            self.beliefs[location] = emissionModel[location] * self.beliefs[location]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        particlePerPos = self.numParticles / len(self.legalPositions)
        for p in self.legalPositions:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:      
            obsProb = self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition()) 
            if particle in beliefs:
                beliefs[particle] += obsProb
            else:
                beliefs[particle] = obsProb
        beliefs.normalize()
        if beliefs.total()  == 0:
    ...
    def getBeliefDistribution(self):
        ...
        """
        dist = DiscreteDistribution()
        n = Counter(self.particles)
        for particle, count in dict(n).iteritems():
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        if len(permutation) < self.numParticles:
            particlePerPos =  self.numParticles / len(permutation)
            for p in permutation:
                for i in range(int(particlePerPos)):
                    self.particles.append(p)
        else:
            for i in range(self.numParticles):
                self.particles.append(permutation[i])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in permutations:
            for i in range(self.numGhosts):
                weight = self.getObservationProb(observation[i], gameState.getPacmanPosition(), p[i], self.getJailPosition(i))
                beliefs[p] *= weight
        ...
        newSample = []
        for i in range(self.numParticles):
            newSample.append(beliefs.sample())
        self.particles = newSample
