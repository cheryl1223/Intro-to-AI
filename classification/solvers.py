...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            new_param_tensor = param_vars[i] - self.learning_rate * grad_tensors[i]
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        updates = []
        "*** YOUR CODE HERE ***"
        v_new = [(vel_vars[i], self.momentum * vel_vars[i] - self.learning_rate * grad_tensors[i]) for i in range(len(vel_vars))]
        w_new = [(param_vars[i], param_vars[i] + v_new[i][1]) for i in range(len(param_vars))]
        updates = v_new + w_new
        return updates
